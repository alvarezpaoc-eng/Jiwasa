...
// ─── Si tienes el video local, descomenta esta línea:
// import cocinaVideo from '../assets/videos/momento-cocina.mp4'
...
const STEPS = [
  {
    n: 1,
    icon: '🥄',
    title: 'Presenta la cuchara',
    text: 'Muestra la cuchara a tu hijo y dale tiempo para observarla sin prisa.',
  },
  {
    n: 2,
    icon: '✋',
    title: 'Invítalo a agarrarla',
    text: 'Anímalo a tomarla por sí solo, aunque tarde un momento. No la pongas en su mano.',
  },
  {
    n: 3,
    icon: '🌀',
    title: 'Mezclen juntos',
    text: 'Permite que mezcle lentamente. No corrijas cada movimiento, solo acompáñalo.',
  },
  {
    n: 4,
    icon: '💛',
    title: 'Celebra el momento',
    text: 'Felicita su esfuerzo con una sonrisa o una frase positiva, sin importar el resultado.',
  },
]

const TIPS = [
  'Si hoy se distrae, está completamente bien.',
  'No necesitas hacerlo perfecto para que valga.',
  'Tu presencia ya es terapia.',
  'Cada pequeño intento cuenta más de lo que imaginas.',
]
...
export default function ActividadRecomendadaPage() {
  const navigate = useNavigate()
    ...
    <div className="min-h-screen bg-beige-soft pb-28">
      ...
      <div className="sticky top-0 z-30 glass border-b border-white/60 px-5 py-4">
        <div className="max-w-lg mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-full bg-yellow-light border border-yellow-warm/30">
            ...
            <span className="text-xs font-semibold text-yellow-mid">{ACTIVITY.duration}</span>
          </div>
        </div>
      </div>
      ...
      >
        ...
        <section className="mb-6">
          <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-blue-soft to-blue-mid shadow-card p-5">
            {/* Decorative blobs */}
            ...
            <div className="relative z-10">
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  <p className="text-white/60 text-xs mb-0.5">Preparada por {ACTIVITY.therapist}</p>
                  <h1 className="text-white font-semibold text-xl leading-tight">{ACTIVITY.title}</h1>
                  <p className="text-white/60 text-xs mt-0.5">{ACTIVITY.subtitle}</p>
                </div>
              </div>

              {/* Goals */}
              <div className="flex flex-wrap gap-2 mb-4">
                {ACTIVITY.goals.map(({ icon, label }) => (
                  <span
                    key={label}
                    className="flex items-center gap-1 px-3 py-1 rounded-full bg-white/15 border border-white/20 text-white text-[11px] font-medium"
                  >
                    {icon} {label}
                  </span>
                ))}
        ...
        <section className="mb-6">
          ...
          <div className="bg-white rounded-3xl border border-beige-mid shadow-soft overflow-hidden">
            {/* 
              VIDEO REAL: descomenta la línea del import arriba y reemplaza el src.
              <video
                ref={videoRef}
                src={cocinaVideo}
                controls
              ...
              />
            */}
            {/* Placeholder mientras no hay video local */}
            <div
            ...
            >
              ...
              <p className="text-xs text-gray-500 font-medium">Video: Momento Cocina</p>
              <p className="text-[11px] text-gray-400">Agrega tu video en src/assets/videos/</p>
            </div>
            <div className="px-4 py-3 border-t border-beige-mid">
              <p className="text-[11px] text-gray-400 text-center leading-relaxed">
                🎥 Una guía breve para ver cómo hacerlo juntos, sin presión
              </p>
            </div>
          </div>
        </section>
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className={`text-xs leading-relaxed transition-all duration-200 ${
                        isActive ? 'text-gray-600 max-h-20 opacity-100' : 'text-gray-400 max-h-0 opacity-0 overflow-hidden'
                      }`}>
                        {text}
                      </p>
                    </div>
        ...
        <section className="mb-6">
          <div className="bg-white rounded-3xl border border-beige-mid shadow-soft p-5">
            ...
            <p className="text-sm text-gray-500 leading-relaxed mb-3">
              Mezclar, agarrar y manipular objetos cotidianos fortalece la coordinación, la
              atención y la confianza de tu hijo de forma natural, sin que parezca una terapia.
            </p>
            <p className="text-sm text-gray-500 leading-relaxed">
              Además, compartir momentos en la cocina profundiza el vínculo emocional y envía un
              mensaje poderoso: <span className="text-blue-soft font-medium">"Tú puedes hacerlo."</span>
            </p>
          </div>
        </section>
        ...
        <section className="mb-6">
          <div className="rounded-3xl border border-yellow-warm/40 bg-gradient-to-br from-yellow-light to-white shadow-soft p-5">
            ...
            <p className="text-xs text-gray-500 leading-relaxed">
              Solo deja que tu hijo mezcle durante unos segundos y celebren juntos ese momento.
              Con eso ya es suficiente. <span className="font-medium text-gray-600">La constancia, no la perfección.</span>
            </p>
          </div>
        </section>
        ...
        <section className="mb-6">
          ...
          <div className="grid grid-cols-2 gap-3">
            {TIPS.map((tip, i) => (
              <div
                key={i}
                className="bg-white rounded-2xl border border-beige-mid shadow-soft p-3.5"
          ...
          <section className="mb-6">
            ...
            <div className="bg-white rounded-3xl border border-beige-mid shadow-soft p-4">
              ...
              <div className="flex gap-3">
                {MOOD_OPTIONS.map(({ emoji, label, value, color }) => (
                  <button
                    key={value}
                  ...
                  >
                    ...
                    <span className={`text-[10px] font-medium text-center leading-snug ${
                      selectedMood === value ? '' : 'text-gray-400'
                    }`}>
                      {label}
                    </span>
                  </button>
              ...
              <div className="grid grid-cols-3 gap-3">
                {[
                  ...
                  >
                    ...
                    {uploadHint === type && (
                      <span className="text-[10px] text-blue-soft">Agregado ✓</span>
                    )}
                  </button>
                ))}
              </div>
